...
public class ExcelTool
{
    ...
    private static void GenerateExcelDataClass(DataTable table)
    {
        ...
        var str = "public class " + table.TableName + "\n{\n";
    ...
    private static void GenerateExcelBinary(DataTable table)
    {
        ...
        //创建一个2进制文件进行写入
        using (var fs = new FileStream(BinaryDataMgr.DATA_BINARY_PATH + table.TableName + ".tang", FileMode.OpenOrCreate, FileAccess.Write))
        {
            //存储具体的excel对应的2进制信息
            ...
            var bytes = Encoding.UTF8.GetBytes(keyName);
            //存储字符串字节数组的长度
            ...
            var rowType = GetVariableTypeRow(table);
            for (var i = BEGIN_INDEX; i < table.Rows.Count; i++)
            {
                //得到一行的数据
                row = table.Rows[i];

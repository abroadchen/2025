...
func main() {
	...
	fmt.Printf("\n file download complate cost: %f seconds\n", time.Now().Sub(starTime).Seconds())
}
...
func (d *FileDownloader) head() (int, error) {
	...
	if resp.StatusCode > 299 {
		return 0, errors.New(fmt.Sprintf("status code error: %d %s", resp.StatusCode, resp.Status))
	}
	if resp.Header.Get("Accept-Ranges") != "bytes" {
		return 0, errors.New(fmt.Sprintf("header error: %d %s", resp.StatusCode, resp.Header.Get("Accept-Ranges")))
	}
...
func (d *FileDownloader) downloadPart(c filePart) error {
	...
	if resp.StatusCode > 299 {
		return errors.New(fmt.Sprintf("status code error: %v %s", resp.StatusCode, resp.Status))
	}
	...
	if len(bs) != (c.To - c.From + 1) {
		return errors.New(fmt.Sprintf("file size error: %d %d", len(bs), c.To-c.From+1))
	}
	c.Data = bs
...
func (d FileDownloader) mergeFileParts() error {
	log.Println("start combine file")
	...
	if totalSize != d.fileSize {
		return errors.New(fmt.Sprintf("file size error: %d %d", totalSize, d.fileSize))
	}

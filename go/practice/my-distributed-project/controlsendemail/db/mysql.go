...
func (mysqlclient *MySQLclient) GetallEmail() []string {
	rows, err := mysqlclient.selfsql.Query("SELECT * FROM log")
	if err != nil {
		return []string{}
	} else {
		...
		for rows.Next() {
			...
			var isok int
			err = rows.Scan(&id, &email, &isok)
			returnstr = append(returnstr, email)
		}
		return returnstr
	}
}
...
func (mysqlclient *MySQLclient) UpdateSendEmailStat(email string, isok int) error {
	stmt, err := mysqlclient.selfsql.Prepare("update log SET Isok=? where Email=?")
	if err != nil {
		return err
	}
	res, err := stmt.Exec(isok, email)
	if err != nil {
		return err
	}
	affect, err := res.RowsAffected()
	fmt.Println("update...", email, affect, err)
	if err != nil {
		return err
	}
	return nil

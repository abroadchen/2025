...
func (etcdkv *ETCDKV) WaitforEvent(key string, myfunc func(value string)) {
	go func() {
		for {
			rch := etcdkv.client.Watch(context.Background(), key)
			for wresp := range rch {
				for _, ev := range wresp.Events {
					myfunc(string(ev.Kv.Value))
				}
			}

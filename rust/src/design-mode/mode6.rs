...
// fn convert_a_to_b3(e: MyEnum) -> MyEnum {//     if let MyEnum::A { name, x: 0 } = e {//         return MyEnum::B{ name: name.clone()}//     } else {//         return e//     }// }
fn main() {
    let mut a = MyEnum::A {
        name: "A".to_string(),
        x: 0,
    ...
    // let a =  convert_a_to_b3(a);    // println!("After convert3, a is {:?}", a);
}
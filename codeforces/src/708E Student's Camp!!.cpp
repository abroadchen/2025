...
f[j]: 第n行第j列的f状态值
a[j-1]: 在k步中向右移动j-1次的概率（二项分布）
s[m-j]: 从位置j到m的累积概率 = Σ(a[t]) for t in [0, m-j]
物理意义：在第j-1列向右移动一定步数，剩余列向右移动任意步数的概率贡献
g[j]: 第n行第j列的g状态值
a[m-j]: 在k步中向右移动m-j次的概率
s[j-1]: 从位置1到j-1的累积概率 = Σ(a[t]) for t in [0, j-1]
物理意义：在第m-j列向右移动一定步数，前面列向右移动任意步数的概率贡献
h[j]: 第n行第j列的h状态值
s[j-1]: 前j-1列的累积概率
s[m-j]: 从j到m列的累积概率
物理意义：在所有位置都有累积效应的概率贡献
f状态：从左向右的贡献
g状态：从右向左的贡献
h状态：两侧累积的贡献
